...
try {
    ...
    Log-INFO "Attente du tunnel Cloudflare MomentDEV et de SERVER_URL..."
    ...
    if (
        ...
        throw "Cloudflare MomentDEV n'est pas pret apres 150 secondes. Expo n'est pas lance pour eviter une ancienne SERVER_URL."
    }

    Log-OK "SERVER_URL DEV synchronisee : $readyUrl"
    Log-INFO "Demarrage Expo MomentDEV sur le port 8082 avec cache nettoye"

    & npx.cmd expo start --tunnel --port 8082 --clear
}
catch {
    Log-KO $_.Exception.Message

    Write-Host ""
    Write-Host "EXPO MOMENTDEV NON DEMARRE" -ForegroundColor Red
}
